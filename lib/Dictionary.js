...
var PATH_DIC_MAIN      = "./dict/main.dic",
   PATH_DIC_SURNAME    = "./dict/surname.dic",
   PATH_DIC_QUANTIFIER = "./dict/quantifier.dic",
   PATH_DIC_SUFFIX     = "./dict/suffix.dic",
   PATH_DIC_PREP       = "./dict/preposition.dic",
   PATH_DIC_STOP       = "./dict/stopword.dic";
...
var Dictionary = {
  ...
  loadMainDict: function(){
		//建立一个主词典实例
		this._MainDict = new DictSegment();

		//读取主词典文件
    var file = fs.readFileSync(PATH_DIC_MAIN, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
	...
	loadSurnameDict: function(){
		//建立 词典实例
		this._SurnameDict = new DictSegment();

		//读取 词典文件
    var file = fs.readFileSync(PATH_DIC_SURNAME, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
	...
	loadQuantifierDict: function(){
		//建立一个量词典实例
		this._QuantifierDict = new DictSegment();

		//读取量词词典文件
    var file = fs.readFileSync(PATH_DIC_QUANTIFIER, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
	...
	loadSuffixDict: function(){
		//建立 词典实例
		this._SuffixDict = new DictSegment();

		//读取 词典文件
    var file = fs.readFileSync(PATH_DIC_SUFFIX, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
	...
	loadPrepDict: function(){
		//建立 词典实例
		this._PrepDict = new DictSegment();

		//读取 词典文件
    var file = fs.readFileSync(PATH_DIC_PREP, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
	...
	loadStopWordDict: function(){
		//建立停词词典实例
		this._StopWords = new DictSegment();

		//读取停词词典文件
    var file = fs.readFileSync(PATH_DIC_STOP, {encoding: 'utf8'});
    file = file.replace(/ |\r/g, '');
    file = file.split('\n');
